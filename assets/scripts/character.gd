...
@onready var wep_anim_player = $WeaponAnimation
@onready var raycast3d = $Head/Camera/RayCast3D
@onready var muzzle_flash = $Head/Camera/Pistol/MuzzleFlash
...
func _physics_process(delta):
	...
	if wep_anim_player.current_animation == "Shoot":
		pass
	elif input_dir != Vector2.ZERO and is_on_floor():
		wep_anim_player.play("Move")
	else:
		wep_anim_player.play("Indle")

func handle_shooting():
	if not is_multiplayer_authority(): return
	
	if Input.is_action_just_pressed("shoot") and wep_anim_player.current_animation != "shoot":
		playShootEffects.rpc()
		if raycast3d.is_colliding():
			var hit_player = raycast3d.get_collider()
			hit_player.damage.rpc_id(hit_player.get_multiplayer_authority())

@rpc("call_local")
func playShootEffects():
	wep_anim_player.stop()
	wep_anim_player.play("Shoot")
...
func damage():
	if health_component:
		health_changed.emit(health_component.receiveDamage())
